  ...
  try {
    const stockData = await Promise.all(
      tickersArr.map(async (ticker) => {
        const url = `https://polygon-api-worker.shahid5ssg.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`;
...
async function fetchReport(data) {
  const messages = [
    {
      role: "system",
      content:
        "You are a trading guru. Given data on share prices over the past 3 days, write a report of no more than 150 words describing the stocks performance and recommending whether to buy, hold or sell. Use the examples provided between ### to set the style your response. Do not use any markdown formatting in your response. Do not use any emojis or exclamation marks. Do not use any words that are not in the examples provided. Do not use any words that are not in the examples provided. Do not use any words that are not in the examples provided.",
    },
    {
      role: "user",
      content: `${data}
  ...
  try {
    const url = "https://ai-worker.shahid5ssg.workers.dev";
